...
def tokenize_and_save(input_file, output_file, case_insensitive=False):
    """
    Tokenisiert den Text der Eingabedatei mittels NLTK-Tokenizer und speichert die durch die Tokenisierung
    entstandenen Tokens in einer Ausgabedatei. Hierbei wird eine ggf. vorhandene Korrektur (in eckigen Klammern)
    ...
    - case_insensitive (bool): Wenn True, werden die Tokens vor dem Speichern in Kleinbuchstaben umgewandelt.
    """
...
if __name__ == "__main__":
    ...
    # über alle Textdateien im Import-Verzeichnis iterieren (ein Unterverzeichnis pro Autor)
            ...
            for file in os.listdir(author_path):
                if file.endswith("-filtered.txt"):
                    ...
                    tokenize_and_save(
                        ...
                        os.path.join(author_path, f"{basename}-tokens.txt"),
                        case_insensitive=True)

                    # Termliste erstellen
                    token_count = 0
                    term_list = set()
                    tokens_file = os.path.join(author_path, f"{basename}-tokens.txt")
                    with open(tokens_file, "r", encoding="utf-8") as token_file:
                        for line in token_file:
                            term_list.add(line.strip())
                            token_count += 1

                    # alphabetisch sortierte Termliste in Datei speichern
                    term_file = os.path.join(author_path, f"{basename}-terms.txt")
                    with open(term_file, "w", encoding="utf-8") as outfile:
                        for term in sorted(term_list):
                            outfile.write(term + "\n")
                    print(f"Es wurden {len(term_list)} Terme in der Datei {term_file} gespeichert.")
                    ttr = len(term_list) / token_count
                    print(f"Type Token Ratio (TTR) von {basename}: {ttr:.3f}")
                    avg_ttr += ttr

    if num_of_files > 0:
        avg_ttr /= num_of_files
        print(f"\ndurchschnittliches TTR über alle Volltexte: {avg_ttr:.3f}")